...
function computeGPA() {
  ...
  if ($('#gradedCredits')) $('#gradedCredits').textContent = gradedCredits;
...
function computeCGPA(course, current) {
  ...
  // Identify current semester being edited to avoid double-counting
  const curYear = Number($('#yearSel')?.value);
  const curSem = Number($('#semSel')?.value);
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    // Skip if this is the current semester being edited/viewed
    if (year === curYear && sem === curSem) continue;
